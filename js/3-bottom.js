    ...
    cartBtn.addEventListener("touchstart", function(event) {
        if (event.targetTouches.length == 1) {　　　　
            ...
            if (!localStorage.token) {
                location.href = "/git/mobileshop/html/register-login.html";
            } else {
                location.href = "/git/mobileshop/html/cart.html";
            }
        };
    })
    homeBtn.addEventListener("touchstart", function(event) {
        if (event.targetTouches.length == 1) {　　　　
            ...
            var touch = event.targetTouches[0];
            // 把元素放在手指所在的位置
            location.href = "/git/mobileshop/index.html"
        };
    })
    accountBtn.addEventListener("touchstart", function(event) {
        if (event.targetTouches.length == 1) {　　　　
            ...
            if (!localStorage.token) {
                ...
                location.href = "/git/mobileshop/html/account.html";
            }
        }
    })